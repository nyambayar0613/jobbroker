...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		//var input	= jQuery('<input type="text" class="txt" name="' + select.attr('name') + '" value="' + select.val() + '" style="width:60px;height:23px;margin-right:5px;" readonly/>').insertAfter(select);
		var input	= jQuery('<input type="text" class="txt" name="' + select.attr('name') + '" value="' + select.val() + '" style="width:60px;height:23px;margin-right:5px;" maxlength="6"/>').insertAfter(select);
		var loc		= '';
		...
		icon.click(function () {
			...
			jQuery('a', colourPicker).click(function () {
				...
				switch(sel_name){
					case 'bgcolor':
						$('#bgimage_preview').html('<dl style="height:100px;background:url() no-repeat top left #'+hex+'"></dl>');
					break;
					case 'bgcolor_begin':
					case 'bgcolor_end':
						var bgcolor_begin = $("input[name='bgcolor_begin']").val();
						if(sel_name=='bgcolor_end'){
							var GradientTypes = $("select[name='bgcolor_direction'] option:selected").val();
							if(user_agent=='IE'){	// 거지같은 IE
								var dl_style = "filter:progid:DXImageTransform.Microsoft.Gradient(GradientType="+GradientTypes+",StartColorStr=#"+bgcolor_begin+",EndColorStr=#"+hex+");";
							} else if(user_agent=='Chrome' || user_agent=='Safari'){	// Chrome/Safari
								var directions = (GradientTypes==1) ? '0% 0%, 100% 0%' : '0% 0%, 0% 100%';
								var dl_style = "background:-webkit-gradient(linear, "+directions+", from(#"+bgcolor_begin+"), to(#"+hex+"));";
							} else if(user_agent=='Firefox'){	 // FF
								var directions = (GradientTypes==1) ? 'left' : 'top';
								var dl_style = "background:-moz-linear-gradient("+directions+", #"+bgcolor_begin+", #"+hex+");";
							} else if(user_agent=='Opera'){	// Opera
								var directions = (GradientTypes==1) ? 'left' : 'top';
								var dl_style = "background:-o-linear-gradient("+directions+", #"+bgcolor_begin+", #"+hex+");";
							}
							$('#bgimage_preview').html("<dl style='height:100px;"+dl_style+"'></dl>");
						}
					break;
					case 'topcolor':
						$('#topimage_preview').html('<dl style="height:100px;background:url() no-repeat top left #'+hex+'"></dl>');
					break;
					case 'topcolor_begin':
					case 'topcolor_end':
						var topcolor_begin = $("input[name='topcolor_begin']").val();
						if(sel_name=='topcolor_end'){
							var GradientTypes = $("select[name='topcolor_direction'] option:selected").val();
							if(user_agent=='IE'){	// 거지같은 IE
								var dl_style = "filter:progid:DXImageTransform.Microsoft.Gradient(GradientType="+GradientTypes+",StartColorStr=#"+topcolor_begin+",EndColorStr=#"+hex+");";
							} else if(user_agent=='Chrome' || user_agent=='Safari'){	// Chrome/Safari
								var directions = (GradientTypes==1) ? '0% 0%, 100% 0%' : '0% 0%, 0% 100%';
								var dl_style = "background:-webkit-gradient(linear, "+directions+", from(#"+topcolor_begin+"), to(#"+hex+"));";
							} else if(user_agent=='Firefox'){	 // FF
								var directions = (GradientTypes==1) ? 'left' : 'top';
								var dl_style = "background:-moz-linear-gradient("+directions+", #"+topcolor_begin+", #"+hex+");";
							} else if(user_agent=='Opera'){	// Opera
								var directions = (GradientTypes==1) ? 'left' : 'top';
								var dl_style = "background:-o-linear-gradient("+directions+", #"+topcolor_begin+", #"+hex+");";
							}
							$('#topimage_preview').html("<dl style='height:100px;"+dl_style+"'></dl>");
						}
					break;
				}

				/* 
					IE GradientType=0  상하
						GradientType=1  좌우
					X IE top 상하
						   left 좌우
					IE  filter:progid:DXImageTransform.Microsoft.Gradient(GradientType=0,StartColorStr=#ffffff,EndColorStr=#f5f5f5);
					FIrefox  background:-moz-linear-gradient(top, #ffffff, #f5f5f5); 
					Opera  background:-o-linear-gradient(top, #ffffff, #f5f5f5);
					Chorome,Safari  background:-webkit-gradient(linear, center top, center bottom, from(#ffffff), to(#f5f5f5));
