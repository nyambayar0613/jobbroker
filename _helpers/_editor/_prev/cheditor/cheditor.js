...
setWinPosition : function (oWin, oWinWidth) {
	...
	T += this.cheditor.toolbarWrapper.offsetHeight;
...
// 이력서 등록시 자기 소개 항목 추가/제거
introduce_checks : function( vals, code ){
	var sel = vals.value;
	var checked = vals.checked;
	//var code_val = $('#'+code).val();

	var input_form = $('.cheditor-editarea-wrapper iframe').contents().find("body");

	if(checked==true){
		input_form.append("<div id='content_"+code+"'>* "+sel+"<br/>- <br/><br/></div>");
	} else {
		$('.cheditor-editarea-wrapper iframe').contents().find('#content_'+code).remove();
	}

},

// 이력서 등록시 자기 소개 항목 전체 선택/제거
introduce_all : function( vals ){
	var checked = vals.checked;
	var introduce_check = $("input[name='wr_introduce_check']");
	var input_form = $('.cheditor-editarea-wrapper iframe').contents().find("body");

	if(checked==true){
		$("input[name='wr_introduce_check']").attr('checked',true);
		...
		$("input[name='wr_introduce_check']").attr('checked',false);
		introduce_check.each(function(){
			var code = $(this).attr('id');
			$('.cheditor-editarea-wrapper iframe').contents().find('#content_'+code).remove();
		});
...
// 알바공고 등록시 상세모집요강 항목 추가/제거
content_checks : function( vals, code ){
	var sel = vals.value;
	var checked = vals.checked;
	//var code_val = $('#'+code).val();
...
// 알바공고 등록시 상세모집요강 항목 전체 선택/제거
content_all : function( vals ){
	var checked = vals.checked;
