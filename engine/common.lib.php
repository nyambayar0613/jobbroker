<?php
define("NFE_URL", "");
define("NFE_PATH", $_SERVER['DOCUMENT_ROOT']);
define("G5_MYSQLI_USE", 0);
######################
/*
: 로그인한 회원서비스 정보
*/
...
include_once NFE_PATH.'/engine/netfu_util.class.php';
include_once NFE_PATH.'/engine/netfu_member.class.php';
include_once NFE_PATH.'/engine/netfu_mjob.class.php';
include_once NFE_PATH.'/engine/netfu_payment.class.php';
$netfu_util = new netfu_util();
$netfu_member = new netfu_member();
$netfu_mjob = new netfu_mjob();
...
function sql_query($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null)
{
    ...
    if(function_exists('mysqli_query') && G5_MYSQLI_USE) {
        ...
        if ($error) {
            $result = @mysql_query($sql) or die("<p>$sql<p>" . mysql_errno() . " : " .  mysql_error() . "<p>error file : {$_SERVER['SCRIPT_NAME']}");
        } else {
...
function sql_num_rows2($result)
{
    if(function_exists('mysqli_num_rows') && G5_MYSQLI_USE)
        return mysqli_num_rows(sql_query($result));
    else
        return mysql_num_rows(sql_query($result));
}
...
function is_field_check($name, $table) {
	return sql_fetch("show columns from ".$table." like '".$name."'");
}
...
{
    global $is_admin;
    if ($is_admin != 'super' && file_exists(NFE_PATH.'/DEMO'))
...
function check_device($device)
{
    ...
    if ($device=='pc' && G5_IS_MOBILE) {
        alert('PC 전용 게시판입니다.', NFE_URL);
    } else if ($device=='mobile' && !G5_IS_MOBILE) {
        alert('모바일 전용 게시판입니다.', NFE_URL);
    }
}
...
function get_skin_stylesheet($skin_path, $dir='')
{
    ...
    $skin_url = NFE_URL.str_replace("\\", "/", str_replace(NFE_PATH, "", $skin_path));
...
// 동일한 host url 인지
function check_url_host($url, $msg='', $return_url=NFE_URL)
{
    if(!$msg)
...
function get_skin_path($dir, $skin)
{
    ...
    if(preg_match('#^theme/(.+)$#', $skin, $match)) { // 테마에 포함된 스킨이라면
        ...
        $theme_path = NFE_PATH.'/'.G5_THEME_DIR.'/'.$cf_theme;
        if(G5_IS_MOBILE) {
...
function get_skin_url($dir, $skin)
{
    ...
    return str_replace(NFE_PATH, NFE_URL, $skin_path);
}
...
{
    if(!$bo_table)
        alert('올바른 방법으로 이용해 주십시오.', NFE_URL);
