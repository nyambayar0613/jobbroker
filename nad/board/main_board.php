<?php
		/*
		* /application/nad/board/main_board.php
		* @author Harimao
		* @since 2015/03/06
		...
		*/
		...
		$print_main = unserialize($get_main['print_main']);
		...
		if($codes) 
			$board_list = $board_config_control->__BoardList("",""," where `code` in (".@implode($codes,", ").") ", " `m_rank` asc " );

		##
		# Include view
		if(is_file($alice['self'] . 'views/main_board.html'))
			include_once $alice['self'] . 'views/main_board.html';
		else
			$config->error_info( $alice['self'] . 'views/main_board.html' );
