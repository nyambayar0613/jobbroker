<?php
		/*
		* /application/nad/member/memo.php
		* @author Harimao
		* @since 2015/03/16
		* @last update 2015/04/14
		* @Module v3.5 ( Alice )
		* @Brief :: Memo list
		* @Comment :: 회원간 쪽지 발송 내역
		*/
		...
		$sub_menu_url = "/" . $alice['admin_member'] . "/memo.php";
		...
		*/
		...
		$con = "";
		$list = $memo_control->__MemoList($page,$page_rows,"",$con);
		$pages = $utility->user_get_paging($page_rows, $page, $list['total_page'],"./memo.php?page_rows=".$page_rows."&page=");
		...
		##
		# Include view
		if(is_file($alice['self'] . 'views/memo.html'))
			include_once $alice['self'] . 'views/memo.html';
		else
			$config->error_info( $alice['self'] . 'views/memo.html' );
