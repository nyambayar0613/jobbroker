<?php
		/*
		* /application/nad/member/custom_employ.php
		* @author Harimao
		* @since 2014/03/26
		...
		*/
		...
		$sub_menu_url = "/" . $alice['admin_member'] . "/custom_employ.php";
		...
		$con .= " and `mb_type` = 'individual' ";

		// individual list
		$member_list = $member_control->__MemberList($page, $page_rows, $con);
		$pages = $utility->get_paging($page_rows, $page, $member_list['total_page'],"./custom_employ.php?".$sorting."page_rows=".$page_rows."&".$member_list['send_url']."&page=");
		...
		##
		# Include view
		if(is_file($alice['self'] . 'views/custom_employ.html'))
			include_once $alice['self'] . 'views/custom_employ.html';
		else
			$config->error_info( $alice['self'] . 'views/custom_employ.html' );
