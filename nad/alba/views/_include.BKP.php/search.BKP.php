...
<input type="hidden" name="page_rows" value="<?php echo $page_rows;?>" id="page_rows"/>
	...
	<tr>
		<td class="ctlt">Бүртгүүлэх огноо</td>
	...
	<tr>
		...
		<td class="pdlnb2" colspan="3">
			...
			<label><input name="wr_service[]" type="checkbox" value="busy" class="check" id="wr_service_busy" <?php echo (@in_array('busy',$wr_service))?'checked':'';?>/>Яаралтай</label>
			<label><input name="wr_service[]" type="checkbox" value="logo" class="check" id="wr_service_logo" <?php echo (@in_array('logo',$wr_service))?'checked':'';?>/>Онцолсон лого</label>
			<label><input name="wr_service[]" type="checkbox" value="neon" class="check" id="wr_service_neon" <?php echo (@in_array('neon',$wr_service))?'checked':'';?>/>Тодруулага</label>
	...
	<tr>
		...
		<td class="pdlnb2">
			...
			<span id="wr_job_type_1_display">
				...
				?>
					<option value="">1-р ажил мэргэжлийн эхлээд сонгоно уу.</option>
				<?php
				}	// if end.
				?>
				</select>
			...
			<span id="wr_job_type_2_display">
				...
				?>
					<option value="">2-р ажил мэргэжлийн эхлээд сонгоно уу./option>
				<?php
				}	// if end.
				?>
				</select>
		...
		<td class="pdlnb2">
			...
			<span id="wr_area1_display">
				<select  style="width:180px;" id="wr_area1" name="wr_area1" title="хороо·дүүрэг сонгох">
	...
	<tr>
		...
		<td class="pdlnb2">
			...
			<label><input name="wr_career_type" type="radio" value="2" class="radio" onclick="career_sel(this);" <?php echo ($wr_career_type=='2')?'checked':'';?>>Дадлага туршлага</label> &nbsp;
			<span id="wr_career_display" style="display: <?php echo ($wr_career_type=='2')?'':'none';?>;">
				...
				<option value=""> Дадлага туршлага сонгох </option>
				<?php
				if($job_career_list) {
			...
			<label><input name="wr_career_type_0" type="checkbox" value="1" class="check" id="wr_career_type_0" <?php echo ($wr_career_type_0)?'checked':'';?>/>Үүнд</label>
		</td>   
	...
	<tr>
		...
		<td class="pdlnb2">
			...
			<label><input type="checkbox" name="wr_volume_always" id="wr_volume_always" value="1" <?php echo ($wr_volume_always)?'checked':'';?>/> Тогтмол элсэлттэй</label>&nbsp;
	...
	<tr>
		...
		<td class="pdlnb2" colspan="3">
			...
			<option value="phone" <?php echo ($wr_requisition=='phone')?'selected':'';?>>утас</option>
			...
			<option value="homepage" <?php echo ($wr_requisition=='homepage')?'selected':'';?>>нүүр хуудас</option>
			</select>&nbsp;
		</td>
	</tr>
	</table>
	<dl class="tc pd7 wbg">
		<select name="search_field" class="s23" id="search_field">
			...
			<option value="wr_content" <?php echo ($_GET['search_field']=='wr_content')?'selected':'';?>>Ажилд авах агуулга</option>
			<option value="wr_phone" <?php echo ($_GET['search_field']=='wr_phone')?'selected':'';?>>Холбогдох дугаар</option>
			<option value="wr_hphone" <?php echo ($_GET['search_field']=='wr_hphone')?'selected':'';?>>Утасны дугаар</option>
			<option value="wr_email" <?php echo ($_GET['search_field']=='wr_email')?'selected':'';?>>И-мэйл хаяг</option>
		</select>
		...
		<span class="btn" id="dsrchType" enum="yes"><input type='button' class="btn23 b" onFocus="blur()" value='Дэлгэрэнгүй хайлт' id="searchBtn"></span>
