<?php
		...
		*/
		...
		$sub_menu_url = "/" . $alice['admin_alba'] . "/employ_end.php";
		...
		$con = $where . " ( ";
		$con .= " `wr_volume_always` = 0 and `wr_volume_end` = 0 ";
		$con .= " and `wr_volume_date` < curdate() ";
		$con .= " ) ";
		...
		$pages = $utility->get_paging($page_rows, $page, $alba_list['total_page'],"./employ_end.php?".$sorting."page_rows=".$page_rows."&".$alba_list['send_url']."&page=");
		...
		##
		# Include view
		if(is_file($alice['self'] . 'views/employ_end.html'))
			include_once $alice['self'] . 'views/employ_end.html';
		else
			$config->error_info( $alice['self'] . 'views/employ_end.html' );
