 ...
 *
 */
class BankSSGWebParamGather implements WebParamGather{
	...
	public function gather($request){
		...
		$webParam->setParameter("BankName", $BankName);
		
		$FnCd = isset($request["FnCd"]) ? $request["FnCd"] : "";
		$webParam->setParameter("FnCd", $FnCd);
		
		$BarCd = isset($request["BarCd"]) ? $request["BarCd"] : "";
		$webParam->setParameter("BarCd", $BarCd);
