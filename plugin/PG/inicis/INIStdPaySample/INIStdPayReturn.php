	...
	return false;
        ...
        try {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                $mid 			= $netfu_payment->use_pg['pg_id'];     					// 가맹점 ID 수신 받은 데이터로 설정
                ...
                try {
					...
					//  && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) 이값이 있으면 결제가 안되는 현상이 있음
					if ((strcmp("0000", $resultMap["resultCode"]) == 0) ){	//결제보안 추가 2016-05-18
				       ...
				       ******************************************************************************/
						...
						if($_REQUEST['merchantData'] && $merchantData_arr[0]) {
							$_REQUEST['mode'] = $merchantData_arr[0];
							$_REQUEST['no'] = $merchantData_arr[1];
						}

						/*디비 처리하기*/
						$result = $payment_control->payment_status($resultMap);
						$_pay_result = true;
						return false;
