...
// li에 class="gold1 gold2"
if($row) {
	$_option = $netfu_mjob->get_service_option('alba', $row);
	$_logo_c = $netfu_mjob->get_logo_type('wr_service_special_main_logo', $row, $get_logo);
?>
<button class="more_option job_more_option" no="<?=$row['no'];?>"><span>더보기</span></button>
<a href="<?=NFE_URL;?>/job/detail.php?no=<?=$row['no'];?>"><div class="logo_box <?=$_logo_c[2];?>" _code="not_js">
	<?=$_logo_c['img'];?>
</div></a>
<div class="bg_">
	<div class="text_box  <?=$_option['bold'];?>">
		<div class="company"><a href="<?=NFE_URL;?>/job/detail.php?no=<?=$row['no'];?>"><?=stripslashes($row['wr_company_name']);?></a></div>
		<div class="title <?=$_option['bold'];?>"><a href="<?=NFE_URL;?>/job/detail.php?no=<?=$row['no'];?>"><?=$_option['icon'];?><span class="<?=$_option['blink'];?>" style="<?=$_option['neon'];?>;<?=$_option['color'];?>;"><?=stripslashes($row['wr_subject']);?></span></a></div>
		<div class="info">
...
<?php } else {?>
<div class="white_box special-wbx">
