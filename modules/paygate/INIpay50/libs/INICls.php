...
*/
  ...
  /* $inipay->SetXPath("INIpay/ReservedInfo/UserDefined10","value10");
	...
	function AddUserDefinedEntity( $info_node, $downnode, $xml, $upnode )
	{
   	foreach ($this->m_REQUEST2 as $key => $val)
   	{
			...
			if( $node_cnt < 3 ) 													continue; //minumum 3
			if( $node[1] != $info_node ) 									continue;
			if( $node_cnt == 3 && $downnode != "" ) 			continue;
			if( $node_cnt == 4 && $downnode != $node[2] ) continue;
  ...
  function MakeBody()
	...
	function ParseHead( $head )
	{
		...
		if( strlen( $head ) != MSGHEADER_LEN ) return RESULT_MSG_FORMAT_ERR;

		$this->m_FlgCrypto						=	substr( $head, $iLen, FLGCRYPTO_LEN );	$iLen += FLGCRYPTO_LEN;
		$this->m_FlgSign 							= substr( $head, $iLen, FLGSIGN_LEN 	);	$iLen += FLGSIGN_LEN;
		$this->m_MPubSN								= substr( $head, $iLen, MPUBSN_LEN 		);	$iLen += MPUBSN_LEN;
		$this->m_PIPGPubSN						= substr( $head, $iLen, PIPGPUBSN_LEN	); 	$iLen += PIPGPUBSN_LEN;
		$this->m_TXPGPubSN						= substr( $head, $iLen, TXPGPUBSN_LEN	); 	$iLen += TXPGPUBSN_LEN;
		$this->m_Cmd 									= substr( $head, $iLen, CMD_LEN 			); 	$iLen += CMD_LEN;
		$this->m_RESULT[NM_MID] 			= substr( $head, $iLen, MID_LEN 			);	$iLen += MID_LEN;
		$this->m_RESULT[NM_TOTPRICE] 	= substr( $head, $iLen, TOTPRICE_LEN 	); 	$iLen += TOTPRICE_LEN;
	...
	{
		$xml = new XML();
		if( ($rtv = $xml->load_xml("",$body)) != OK ) return $rtv;
	...
	{
		$xml = new XML();
		if( ($rtv = $xml->load_xml("",$decrypted)) != OK ) return $rtv;
...
class INICrypto
{
  ...
  function INICrypto( $request )
  {
	...
	function LoadPGPubKey( &$pg_pubcert_SN )
	{
		...
		if( !$fp ) return NULL_PGCERT_FP_ERR;
		...
		if ( !$this->pgpubkeyid ) return NULL_PGCERT_ERR;

		$pg_pubcert = openssl_x509_parse($pub_key);
		if( !$pg_pubcert ) return NULL_X509_ERR; //The structure of the returned data is (deliberately) not yet documented
	...
	function UpdatePGPubKey( $pgpubkey )
	{
		...
		$fp=fopen( $f_new, "w");
		if( !$fp ) return PGPUB_UPDATE_ERR;
		fwrite($fp, $pgpubkey);
		fclose($fp);

		//rename
		if( !rename( $f_new, $f_org ) ) return PGPUB_UPDATE_ERR; 
		return OK;
	}		
	function LoadMPubKey( &$m_pubcert_SN )
	{
		...
		if( !$fp ) return NULL_MCERT_FP_ERR;
		$pub_key=fread($fp, 8192);
		if( !$pub_key ) 
		{
			fclose($fp);
		...
		if( !$m_pubcert ) return NULL_X509_ERR; //The structure of the returned data is (deliberately) not yet documented
    ...
    fclose($fp);
		...
		*/
		...
		if( !$fp ) return PRIVKEY_FILE_OPEN_ERR;
	...
	function Decrypt( $sessionkey, $encrypted, &$decrypted )
	{
		...
		if( !$this->RSAMPrivDecrypt( $src, $key ) ) return DEC_RSA_ERR;
    ...
    $src_data = $this->pkcs5_pad($src_data, $size);
		...
		if( !$enc_data ) return ENC_FINAL_ERR;
	...
	function FreePubKey()
	{
		if( $this->pgpubkeyid ) openssl_free_key($this->pgpubkeyid);
	}
	function FreePrivKey()
	{
		if( $this->mprivkeyid ) openssl_free_key($this->mprivkeyid);
	}
	function FreeAllKey()
	{
		$this->FreePubKey();			
