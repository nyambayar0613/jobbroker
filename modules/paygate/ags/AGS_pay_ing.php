...
********************************************************************************/
	...
	$get_payment = $payment_control->get_payment_for_oid($_POST['OrdNo']);
	...
	****************************************************************************/
	...
	$agspay->SetValue("AgsPayHome",$_SERVER['DOCUMENT_ROOT'] . "/modules/paygate/ags");			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	if($get_payment['pay_service']=='etc_open'){	// 이력서 열람 서비스 일때만
		$agspay->SetValue("ProdNm",trim(iconv("UTF-8","EUC-KR",$_POST["ProdNm"])));			//상품명
	} else {
		$agspay->SetValue("ProdNm",trim(iconv("UTF-8","EUC-KR",$_POST["ProdNm"])));			//상품명
	}
	...
	/*신용카드사용*/
	$agspay->SetValue("OrdNm",trim(iconv("UTF-8","EUC-KR",$_POST["OrdNm"])));			//주문자명
	...
	$agspay->SetValue("RcpNm",trim(iconv("UTF-8","EUC-KR",$_POST["RcpNm"])));			//수신자명
	...
	****************************************************************************/
	...
	/*
	echo "<pre>";
	echo $agspay->GetResult("rSuccYn")." <==<Br/>";
	//echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rResMsg") )." <==<Br/>";
	print_R($_POST);
	print_R($agspay);
	exit;
	*/
	...
	if($agspay->GetResult("rSuccYn") == "y") { 
		...
		if($agspay->GetResult("AuthTy") == "virtual"){
			...
			// 01. 공고(이력서) 기간 업데이트 및 결제 정보 업데이트 (포인트 지급/차감)
			$payment_control->success_payment( $oid );
						
			// 02. 문자 발송

		   $success_url = "http://".$_SERVER['HTTP_HOST']."/service/success.php?oid=".$oid;
		   echo "<script>location.href='".$success_url."';</script>";

			//echo "고객님의 결제요청이 성공되었습니다.\n";
			$result = "<script>\n";
			//$result .= "var openwin = window.open(\"".$alice['paygate_path']."/ags/AGS_progress.html\",\"popup\",\"width=300,height=160\");\n"; 
			//$result .= "openwin.close();\n";
			$result .= "location.replace(\"".$success_url."?oid=".$oid."\");\n";
			$result .= "</script>\n";
			echo $result;
			exit;
		...
		$no = $_POST['no'];
		$utility->popup_msg_js("결제중 오류가 발생하였습니다.\n\n서비스를 다시 선택해 주세요.",$return_url."?no=".$no);
...
<body onload="javascript:location.replace('<?php echo $success_url."?oid=".$oid;?>');"><!-- frmAGS_pay_ing.submit(); -->
