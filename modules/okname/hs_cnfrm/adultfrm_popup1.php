<?php
	 ...
	 * okname 본인확인서비스 파라미터
	 **************************************************************************/
	$inTpBit			= 0;		// 입력구분코드(0:없음, 1:기본정보, 2:내외국인, 4:휴대폰정보)
	$name			= "x";				// 성명
	$birthday		= "x";			// 생년월일 
	$gender			= "x";			// 성별
	$nation			= "x";			// 내외국인구분 
	...
	if (($inTpBitVal & 1) == 1) {
		$name = iconv('UTF-8','EUC-KR',$_POST["name"]); // 성명
	}
	...
	// # 운영전환시 확인 필요
	// ########################################################################
	$memId = $env['hphone_id'];							// 회원사코드

	$clientIp = $_SERVER['SERVER_ADDR'];			// 모듈설치 서버에 설정된 IP
	$clientDomain = $_SERVER['HTTP_HOST'];		// 회원사 도메인. (휴대폰인증번호 발송시 제휴사명에 노출)
	...
	$returnUrl = $_http."://" . $_SERVER['HTTP_HOST'] . "/modules/okname/hs_cnfrm/adultfrm_popup2.php";// 본인인증 완료후 리턴될 URL (도메인 포함 full path)
	...
	// ########################################################################
	$exe = $alice['okname_gblib_abs_path'] . "/okname";				// okname 실행 정보
