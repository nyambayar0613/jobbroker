<?php
	...
	*/
	...
	$_arr = $field;
	$_arr['name'] = iconv("CP949", "UTF-8", $field['6']);
	$_arr['birth'] = $field['11'];
	$_arr['di'] = $field['0'];
	$_arr['ci'] = $field['1'];
	$_arr['hphone'] = "";
	$_arr['gender'] = !$field['9'] ? 1 : 0;
	...
	if($ret == 0) {
		// : 성인인증
		if($env['use_adult']) {
			if($netfu_util->get_age(substr($field['11'],0,4))<20) {
				echo "<script>alert('성인이 아닙니다.\n\n성인만 접근 가능합니다.');self.close();</script>";
				exit;
			}
		}
	...
	// : 인증실패
		...
		echo "<script>alert('인증결과복호화 실패되었습니다. 다시 진행해주시기 바랍니다.');self.close();</script>";
		exit;
	}
...
function fncOpenerSubmit() {
	...
	opener.document.kcbOutForm.action = "alba_adult.php";
