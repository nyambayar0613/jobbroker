<?php
	...
	*/
	...
	$member_info = $db->query_fetch(" select * from `alice_member` where `mb_ssn` = '".$field[0]."' ");
	...
	if($member_info['mb_id']){	 // 가입된 정보 조회가 된다면 비밀번호 찾기 메일을 발송한다.
		...
		echo "win_open('".$alice['app_path']."/popup/find_id.php?mb_id=".$member_info['mb_id']."', 'pop_findid', 680, 200);window.close();</script>";
		exit;

	} else {	 // 가입된 정보 조회가 안된다면

		echo "<script>alert('".iconv('UTF-8','EUC-KR',"가입된 정보가 없습니다.")."');window.close();</script>";
		exit;
	...
	/*
	// 회원 가입 중복 체크
	$ssn_check = $user_control->ssn_checking($field[4]);
	if($ssn_check){
		echo "<script>alert('".iconv('UTF-8','EUC-KR','이미 가입 하셨습니다.')."');window.opener.location.href = '".$alice['member_path']."/login.php';self.close();</script>";
	}
	*/
